...
RULES_CATALOG: dict[str, RuleMetadata] = {
    ...
    # === Kubernetes rules ===
    "K8S-001": RuleMetadata("K8S-001", "No securityContext on container", "kubernetes",
        ["CIS K8s 5.2.1", "NIST 800-190", "PCI-DSS 2.2"], "Containers must define a securityContext."),
    "K8S-002": RuleMetadata("K8S-002", "Container runs as root", "kubernetes",
        ["CIS K8s 5.2.6", "NIST 800-190", "PCI-DSS 7.1"], "Never run containers as UID 0."),
    "K8S-003": RuleMetadata("K8S-003", "Privileged container", "kubernetes",
        ["CIS K8s 5.2.1", "NIST 800-190"], "Privileged mode disables all security boundaries."),
    "K8S-004": RuleMetadata("K8S-004", "No resource limits", "kubernetes",
        ["CIS K8s 5.4.1"], "Without limits, a pod can consume the entire node."),
    "K8S-005": RuleMetadata("K8S-005", "No readOnlyRootFilesystem", "kubernetes",
        ["CIS K8s 5.2.4"], "Writable root filesystem enables runtime modification of binaries."),
    "K8S-006": RuleMetadata("K8S-006", "Capabilities not dropped", "kubernetes",
        ["CIS K8s 5.2.7", "NIST 800-53 AC-6"], "Drop ALL capabilities and add back only what's needed."),
    "K8S-007": RuleMetadata("K8S-007", "Image uses :latest tag", "kubernetes",
        ["CIS K8s 5.5.1", "SLSA Level 1"], "Mutable tags break deployment reproducibility."),
    "K8S-008": RuleMetadata("K8S-008", "Image has no tag", "kubernetes",
        ["CIS K8s 5.5.1", "SLSA Level 1"], "Implicit :latest."),
    "K8S-009": RuleMetadata("K8S-009", "Public LoadBalancer without annotation", "kubernetes",
        ["CIS K8s 5.3.1"], "Services may be unintentionally internet-facing."),

    # === Docker Compose rules ===
    "COMPOSE-001": RuleMetadata("COMPOSE-001", "Privileged container in Compose", "docker-compose",
        ["CIS Docker 5.4", "NIST 800-190"], "Full host access."),
    "COMPOSE-002": RuleMetadata("COMPOSE-002", "Secret in environment", "docker-compose",
        ["PCI-DSS 3.4", "SOC2 CC6.1"], "Secrets in plain text environment variables."),
    "COMPOSE-003": RuleMetadata("COMPOSE-003", "Host network mode", "docker-compose",
        ["CIS Docker 5.9"], "Bypasses container network isolation."),
    "COMPOSE-004": RuleMetadata("COMPOSE-004", "Dangerous volume mount", "docker-compose",
        ["CIS Docker 5.5", "NIST 800-190"], "Mounting / or docker.sock gives container host control."),
    "COMPOSE-005": RuleMetadata("COMPOSE-005", "No healthcheck", "docker-compose",
        ["CIS Docker 6.1"], "Orchestrator cannot detect unhealthy services."),
    "COMPOSE-006": RuleMetadata("COMPOSE-006", "Image uses :latest", "docker-compose",
        ["CIS Docker 5.8", "SLSA Level 1"], "Non-reproducible deployment."),
    "COMPOSE-007": RuleMetadata("COMPOSE-007", "Dangerous capability added", "docker-compose",
        ["CIS Docker 5.3", "NIST 800-53 AC-6"], "SYS_ADMIN/ALL/NET_ADMIN overgrant capabilities."),

    # === Shell script rules ===
    "SHELL-001": RuleMetadata("SHELL-001", "eval usage", "shell",
        ["CIS Linux 1.1", "OWASP A03:2021"], "Code injection if input is untrusted."),
    "SHELL-002": RuleMetadata("SHELL-002", "curl pipe to shell", "shell",
        ["OWASP Supply Chain", "SLSA Level 1"], "Supply chain attack vector."),
    "SHELL-003": RuleMetadata("SHELL-003", "Unquoted variable in rm", "shell",
        ["CIS Linux 1.1"], "Empty variable expands to 'rm -rf /' disaster."),
    "SHELL-004": RuleMetadata("SHELL-004", "chmod 777", "shell",
        ["CIS Linux 1.1", "PCI-DSS 7.1"], "World-writable permissions."),
    "SHELL-005": RuleMetadata("SHELL-005", "Hardcoded credential", "shell",
        ["PCI-DSS 3.4", "SOC2 CC6.1"], "Credentials in plain text scripts."),
    "SHELL-006": RuleMetadata("SHELL-006", "Predictable temp file path", "shell",
        ["CIS Linux 1.1"], "Symlink attack risk. Use mktemp."),
    "SHELL-007": RuleMetadata("SHELL-007", "sudo in script", "shell",
        ["CIS Linux 5.6"], "Scripts with sudo may fail in restricted environments."),
    "SHELL-008": RuleMetadata("SHELL-008", "Missing set -e", "shell",
        ["CIS Linux 1.1"], "Script continues after errors silently."),
    "SHELL-009": RuleMetadata("SHELL-009", "Missing set -u", "shell",
        ["CIS Linux 1.1"], "Unset variables don't cause errors."),
    "SHELL-010": RuleMetadata("SHELL-010", "Missing set -o pipefail", "shell",
        ["CIS Linux 1.1"], "Pipe failures are silently ignored."),
}
