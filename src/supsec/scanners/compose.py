    ...
    def scan(self, path: Path) -> list[Finding]:
        ...
        for svc_name, svc in services.items():
            ...
            for entry in env_list:
                entry_str = str(entry)
                import re
                if re.search(r"(PASSWORD|SECRET|API_KEY|TOKEN)\s*=\s*[^\$].{5,}", entry_str, re.IGNORECASE):
                    if "${" not in entry_str and "$$" not in entry_str:
                        findings.append(Finding(
                            rule_id="COMPOSE-002",
                            severity=Severity.HIGH,
                            file=rel, line=self._find_line(lines, entry_str.split("=")[0]),
            ...
            if svc.get("network_mode") == "host":
                findings.append(Finding(
                    rule_id="COMPOSE-003",
                    severity=Severity.HIGH,
                    file=rel, line=self._find_line(lines, "network_mode"),
            ...
            for vol in volumes:
                ...
                if vol_str.startswith("/:/") or vol_str.startswith("/var/run/docker.sock"):
                    findings.append(Finding(
                        rule_id="COMPOSE-004",
                        severity=Severity.CRITICAL,
                        file=rel, line=self._find_line(lines, vol_str.split(":")[0]),
            ...
            if "healthcheck" not in svc:
                findings.append(Finding(
                    ...
                    file=rel, line=self._find_line(lines, svc_name),
            ...
            for cap in cap_add:
                if cap in ("ALL", "SYS_ADMIN", "NET_ADMIN"):
                    findings.append(Finding(
                        rule_id="COMPOSE-007",
                        severity=Severity.HIGH,
                        file=rel, line=self._find_line(lines, cap),
