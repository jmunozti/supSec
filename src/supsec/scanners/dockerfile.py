...
class DockerfileScanner(BaseScanner):
    ...
    def scan(self, path: Path) -> list[Finding]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if re.search(r"curl.*\|\s*(ba)?sh", stripped) or re.search(r"wget.*\|\s*(ba)?sh", stripped):
                findings.append(Finding(
                    rule_id="DOCKER-005",
                    severity=Severity.HIGH,
                    file=rel, line=i,
            ...
            if upper.startswith("FROM ") and ":" not in stripped.split()[-1] and " AS " not in upper:
        ...
        if not has_user:
            findings.append(Finding(
                rule_id="DOCKER-010",
                severity=Severity.HIGH,
                file=rel, line=last_from_line or 1,
