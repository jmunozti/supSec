...
SECRET_PATTERNS = [
    ...
    ("Private Key Header", r"-----BEGIN (RSA |EC |DSA |OPENSSH )?PRIVATE KEY-----", Severity.CRITICAL),
    ("Generic password assignment", r"(?i)(password|passwd|pwd)\s*[=:]\s*['\"][^'\"$]{8,}['\"]", Severity.HIGH),
    ("Generic API key assignment", r"(?i)(api[_-]?key|apikey)\s*[=:]\s*['\"][^'\"$]{10,}['\"]", Severity.HIGH),
    ("Generic secret assignment", r"(?i)(secret|token)\s*[=:]\s*['\"][^'\"$]{10,}['\"]", Severity.HIGH),
]
...
class SecretsScanner(BaseScanner):
    ...
    def scan(self, path: Path) -> list[Finding]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if re.search(r"(REPLACE_ME|CHANGE_ME|your[-_]?key|xxx|dummy|example|placeholder)", stripped, re.IGNORECASE):
                continue
            ...
            for token in tokens:
                if len(token) >= self.MIN_HIGH_ENTROPY_LEN and _shannon_entropy(token) >= self.ENTROPY_THRESHOLD:
